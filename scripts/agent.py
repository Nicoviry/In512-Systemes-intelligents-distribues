...
class Agent:
    ...
    def move_randomly(self):
        print('in')
        """ Method to move the agent randomly within the environment """
        directions = [0, 1, 2, 3, 4, 5, 6, 7, 8]
        #int(input("0 <-> Stand\n1 <-> Left\n2 <-> Right\n3 <-> Up\n4 <-> Down\n5 <-> UL\n6 <-> UR\n7 <-> DL\n8 <-> DR\n"))
        sleep(0.8)

        print("Cell Value : ", self.cell_val)

        val = choice(directions)  # Randomly select a direction
    ...
    def agent1(self):
        if self.agent_id == 1:
            var = self.cell_val
            #print(self.matrice)
            descente = np.abs(self.y-(self.h))-3
            while(1):
                cmds = {"header":MOVE}
                for i in range(descente):
                    print(i, self.h, self.y)
                    cmds["direction"] = DOWN
                    sleep(0.1)
...
if __name__ == "__main__":
    ...
    try:    #Manual control test0
        while True:
            ...
            """
            cmds = {"header": 2}
            cmds["direction"] = agent.move_randomly()
